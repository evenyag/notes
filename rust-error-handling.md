# Rust 错误处理的体会
错误处理并非一件容易的事情，尽管在使用 Rust 后，有编译器不厌其烦地督促我们，基本不存在不处理错误的情况了，但这其实并不意味着错误处理变简单了。
...
错误处理中真正麻烦的，还是处理可恢复的错误。

## 定义 Error
在进行错误处理，首先，你得把自己错误给定义了。我认为，对于一个新项目来说，定义好自己的 Error 类型甚至是属于最先要做的几件事情之一。即便一开始不做，等到你写到了第一个 Result 时，你也不得不考虑了。定义 Error 类型是一个可简单，可复杂的事情，毕竟在 `Result<T, E>` 里，`E` 其实可以塞任何东西。如果你胆子够大，甚至可以直接把 String 作为 Error 来使用，还能带上一定的错误信息。
```rust
